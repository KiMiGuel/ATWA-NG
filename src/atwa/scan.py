...
# Locally administered / reserved group addresses that show up as a source
# or destination MAC on real captures but are never an associated station.
# 2026-09-25: live scan reported a "client" of 01:80:c2:00:00:00 on two
# NETGEAR APs. That is the reserved WPS/P2P group address (IEEE 802.1 /
# Wi-Fi Protected Setup), which APs emit in their own management and WPS
# frames -- counting it as a client made the GUI and CLI report a device
# that does not exist, and gave deauth/PINCEK target selection nothing
# real to aim at.
RESERVED_CLIENT_MACS = frozenset({
    "01:80:c2:00:00:00",  # WPS / P2P group address
    "00:00:00:00:00:00",
})


def _is_real_client(addr: str | None, bssid: str | None, own_mac: str | None) -> TypeGuard[str]:
    """True when addr can plausibly be a station associated to bssid.

    Returns a TypeGuard so callers get the non-None narrowing they need to
    index ap.clients (a set[str]) and ap.client_signal (a dict[str, int])
    without a redundant assert.
    """
    ...
    if bssid and lowered == bssid.lower():
        return False
    return not (own_mac and lowered == own_mac.lower())
...
class AccessPoint:
    ...
    pairwise_cipher: str | None = None  # CCMP | TKIP | unknown when RSN exposes it
...
def process_packet(
    raw: bytes, result: ScanResult, own_mac: str | None = None, frame: Frame | None = None,
) -> Frame | None:
    ...
    if is_beacon_or_probe_resp(frame):
        ...
        ies = list(walk_ies(frame.body[12:]))
        ssid = ssid_of(frame, ies)
        if ssid:
            ap.ssid = ssid
        ch = channel_of(frame, ies)
        if ch:
            ap.channel = ch
        profile = security_profile(frame, ies)
        ...
        if profile["security"] != "open" or ap.security is None:
            ...
            ap.pmf = profile["pmf"]
        if profile.get("pairwise_cipher"):
            ap.pairwise_cipher = profile["pairwise_cipher"]
        if ap.security == "OWE":
    ...
    if frame_bssid and frame_bssid in result.aps:
        ...
        for addr in client_candidates:
            if _is_real_client(addr, frame_bssid, own_mac):
                ap.clients.add(addr)
