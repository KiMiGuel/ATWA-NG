...
class OneShot:
    ...
    def _init_wpa_supplicant(self) -> None:
        ...
        while time.monotonic() < deadline:
            ...
            self.wpas.terminate()
            raise RuntimeError("wpa_supplicant did not create its control socket within 10 seconds")
    ...
    def _read_events(
        self, pixiemode: bool = False, pbc_mode: bool = False, timeout: float = 120.0,
    ) -> Iterator[str]:
        """Yield raw wpa_supplicant lines until the session ends or times out."""
        stdout = self._stdout()
        fileno = stdout.fileno()
        ...
        while True:
            ...
            if not ready:
                if time.monotonic() >= deadline:
                    raise RuntimeError(f"wpa_supplicant event read timed out after {timeout:.0f}s")
                continue
            line = stdout.readline()
            if not line:
                self.wpas.wait()
