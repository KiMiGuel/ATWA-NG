"""Live scanning engine. A background thread drives the compiled scan
engine and polls its CSV output every 200ms into a shared,
lock-protected buffer that callers read from independently (a
Loop A / Loop B split, so display/consumption never blocks on the
poll). Points at our own vendored/locally-compiled binary instead of a
PATH lookup, and returns the typed dataclasses from scan_engine.py
...
def numbered_scan_csv_paths(prefix: str) -> list[Path]:
...
def latest_scan_csv_path(prefix: str) -> Path | None:
    matches = numbered_scan_csv_paths(prefix)
    if not matches:
        return None
    return max(matches, key=lambda p: (p.stat().st_mtime, p.name))


def numbered_scan_output_paths(prefix: str) -> list[Path]:
    prefix_path = Path(prefix)
    stem = re.escape(prefix_path.name)
...
def clear_scan_outputs(prefix: str) -> None:
    """The scan engine bumps the -NN suffix instead of overwriting; clear
    stale outputs so a fresh run starts at -01 and pollers don't pick up
    a dead file from a previous session."""
    for path in numbered_scan_output_paths(prefix):
        try:
            path.unlink()
        except OSError:
            pass
...
class ScanEngineWorker(threading.Thread):
    """Drives our vendored scan engine and parses its CSV output live —
    same threading/pause/lock-capture design, same 200ms poll loop."""
    ...
    def _build_base_cmd(self, prefix: str) -> list[str]:
        if not HOPSCAN_BIN.exists():
            raise ScanEngineNotBuilt(f"{HOPSCAN_BIN} not built — see ATWA-NG/STATUS.md")
        return [
            str(HOPSCAN_BIN),
    ...
    def _launch(self, cmd: list[str]) -> tuple[bool, str | None]:
        self._stop_process()
        clear_scan_outputs(self._prefix)
        try:
    ...
    def run(self):
        ...
        shared buffer. Channel hopping/capture is the scan engine's own job;
        this thread only reads its output."""
        last_mtime = 0.0
        last_csv_path = None
        poll_interval = 0.2
        while not self._shutdown.is_set():
            csv_path = latest_scan_csv_path(self._prefix)
                ...
                try:
                    ...
                    if csv_path != last_csv_path or mtime != last_mtime:
                        ...
                        with self._data_lock:
                            self._latest = parse_scan_csv(text)
                except Exception as e:
                    self.queue.put(("error", str(e)))
...
def scan_live(iface: str, duration: float = 10.0, band: str = "Both",
    ...
    scan_engine.py), return whatever was captured.
    ...
    scanner = ScanEngineWorker()
    ok, err = scanner.start_scan(iface, band, prefix)
    if not ok:
        if tmpdir:
            shutil.rmtree(tmpdir, ignore_errors=True)
        raise RuntimeError(f"failed to start scan engine: {err}")
    try:
        time.sleep(duration)
