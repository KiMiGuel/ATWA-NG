...
def _wait_for(iface: str, bssid: str, client: str, matches, timeout: float, send_fn=None, stop_event=None):
    ...
    def handler(pkt):
        ...
        if parsed and parsed.opcode == eap.WSC_OP_MSG:
            pending = fragments.setdefault(parsed.identifier, [])
            pending.append(parsed)
            complete = eap.reassemble_wsc_message(pending)
            if complete is not None:
                fragments.pop(parsed.identifier, None)
                if matches(complete):
                    found.append(complete)
        elif parsed and matches(parsed):
            found.append(parsed)
        ...
        for pin8 in oui_pins_fn(bssid, ssid):
            ...
            if outcome.outcome is AttemptOutcome.TIMEOUT:
                ...
                if consecutive_timeouts >= max_consecutive_timeouts:
                    log(f"{consecutive_timeouts} consecutive timeouts during OUI pre-attempts — aborting")
