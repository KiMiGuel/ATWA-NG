...
def derive_pmk(passphrase: str, ssid: str) -> bytes:
    ...
    if not passphrase.isascii():
        raise ValueError("WPA-PSK passphrase must contain ASCII characters only")
    return hashlib.pbkdf2_hmac("sha1", passphrase.encode("ascii"), ssid.encode("utf-8"), 4096, 32)
