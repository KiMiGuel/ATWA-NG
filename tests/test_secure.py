...
from atwa.dissect import dissect
...
def _frame(pkt):
    """security_profile()/wps_profile()/owe_transition_info() now take a
    dissect.Frame (raw-bytes dissection, the dpkt swap) instead of a
    scapy Packet -- fixtures are still built with scapy's craft_*()
    helpers/hand-built Raw payloads, then run through the real dissect()
    exactly like production code would for a captured frame."""
...
def test_security_profile_open():
    ...
    assert security_profile(_frame(pkt)) == {"security": "open", "pmf": "none"}
...
def test_security_profile_wpa2_psk():
    ...
    profile = security_profile(_frame(pkt))
...
def test_owe_transition_info_extracts_bssid_and_ssid():
    ...
    assert owe_transition_info(_frame(pkt)) == {"bssid": "11:22:33:44:55:66", "ssid": "HomeOpen"}


def test_owe_transition_info_returns_none_when_absent():
    pkt = RadioTap() / Dot11(type=0, subtype=8, addr1="ff:ff:ff:ff:ff:ff", addr2="aa:bb:cc:dd:ee:ff", addr3="aa:bb:cc:dd:ee:ff") / Dot11Beacon()
    assert owe_transition_info(_frame(pkt)) is None


def test_owe_transition_info_handles_empty_ssid():
    pkt = _beacon_with_owe_transition_ie("11:22:33:44:55:66", "")
    assert owe_transition_info(_frame(pkt)) == {"bssid": "11:22:33:44:55:66", "ssid": None}
