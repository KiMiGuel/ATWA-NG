...
class ParsedEap:
    ...
    total_length: int | None = None  # WSC first-fragment total length, when LF is set


def reassemble_wsc_message(fragments: list[ParsedEap]) -> ParsedEap | None:
    """Reassemble inbound WSC fragments, or return None while incomplete."""
    if not fragments:
        return None
    first = fragments[0]
    if not (first.flags & WSC_FLAG_MF) and len(fragments) == 1:
        return first
    if first.identifier != fragments[-1].identifier or any(
        f.identifier != first.identifier or f.opcode != first.opcode for f in fragments[1:]
    ):
        return None
    payload = b"".join(f.payload for f in fragments)
    if first.total_length is None or len(payload) < first.total_length:
        return None
    return ParsedEap(
        first.code, first.identifier, first.eap_type, first.opcode,
        first.flags & ~WSC_FLAG_MF, payload[:first.total_length], first.total_length,
    )
    ...
    if eap_type == EAP_TYPE_EXPANDED:
        ...
        if flags & WSC_FLAG_LF:
            if len(body) < offset + 2:
                return None
            total_length = int.from_bytes(body[offset:offset + 2], "big")
            offset += 2
        tlv = body[offset:]
        return ParsedEap(code, identifier, eap_type, opcode, flags, tlv, total_length)
